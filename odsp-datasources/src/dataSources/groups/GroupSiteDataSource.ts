...
export class GroupSiteDataSource extends SiteCreationDataSource implements IGroupSiteDataSource {
  ...
  public getGroupCreationContext(): Promise<IGroupCreationContext> {
    ...
    return this.getDataUtilizingCache<IGroupCreationContext>({
      ...
      parseResponse: (responseText: string): IGroupCreationContext => {
        let isClassificationDescriptionsFlightEnabled: boolean = Features.isFeatureEnabled(EnableClassificationDescriptions);
        ...
        if (result && result.d && result.d.GetGroupCreationContext) {
          ...
          return {
            ...
            defaultClassification: isClassificationDescriptionsFlightEnabled ? groupCreationContext.DefaultClassification : undefined,
            classificationDescriptions: isClassificationDescriptionsFlightEnabled ?
              this._parseClassificationDescriptions(groupCreationContext.ClassificationDescriptions) : undefined
          };
        }
        return undefined;
      },
  ...
  private _parseClassificationDescriptions(classificationDescriptions: any): IClassificationDescriptionItem[] {
    if (classificationDescriptions && classificationDescriptions.results) {
      return classificationDescriptions.results.map((classificationDescription: any, index: number) => {
        let classificationDescriptionItem: IClassificationDescriptionItem = {
          classificationTitle: classificationDescription.Key,
          classificationDescription: classificationDescription.Value
        }
