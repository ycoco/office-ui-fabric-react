...
import { IShareStrings, ClientId, ShareType, DefaultMailClient } from '../../../interfaces/SharingInterfaces';
...
export class ShareTargets extends React.Component<IShareTargetsProps, {}> {
    ...
    private _getShareTargetsData(): Array<IShareTargetData> {
        ...
        const defaultMailClient = this._determineMailShareTarget(this.props.clientId);
        if (defaultMailClient === DefaultMailClient.outlook) {
            targets.push({
        ...
        } else if (defaultMailClient === DefaultMailClient.nonOutlook) {
            targets.push({
    ...
    private _determineMailShareTarget(clientId: ClientId): DefaultMailClient {
        /**
         * - Outlook share target is always enabled for ODSP.
         * - Partners have the option to implement IsSendViaOutlookEnabled.
         */
        if (ClientIdHelper.isODSP(this.props.clientId)) {
            return DefaultMailClient.outlook;
        } else {
            try {
                const externalJavaScript: any = window.external;
                const defaultMailClient: DefaultMailClient = externalJavaScript.IsSendViaOutlookEnabled();
                return defaultMailClient;
            } catch (error) {
                // If "IsSendViaOutlookEnabled" hasn't been implemented, then don't show the target.
                return DefaultMailClient.none;
            }
        }
    }
